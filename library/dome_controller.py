...
class DomeController:
    def __init__(
        ...
        self.tolerance_counts = max(1, int(self.counts_per_degree * tolerance_degrees))
        ...
        GPIO.add_event_detect(self.encoder_a_pin, GPIO.BOTH, callback=self._enc_callback, bouncetime=1)
        # optional: detect zero index asynchronously
        GPIO.add_event_detect(self.zero_pin, GPIO.BOTH, callback=self._zero_callback, bouncetime=50)
    ...
    def home(self, search_direction='ccw', speed=80, timeout=20.0):
        """
...
if __name__ == "__main__":
    # example usage
    dc = DomeController(counts_per_rev=4096, kp=0.05, min_speed=20, max_speed=200)
